...
app.get("/", (req, res) => {
    res.json({
        ...
        "/data?day=<day>&time=<time>": "Shows data that contains the given day"+
         "<day> at the time <time>.",
        "/data?month=<month>&day=<day>&time=<time>": "Shows data that contains"+
         "the given month <month> on the day <day> at the time <time>."
    });
});
...
app.get("/data", (req, res) => {
    ...
    let maxQueryParam = Object.keys(req.query).length;
    ...
    switch (maxQueryParam) {
        case 1:
            if (req.query.ip) {
                ...
                result = itemsfile.filter(
                    item => item.time.substring(0, 2).includes(req.query.time)
                );
            } else if (req.query.time.length === 5) {
                console.log("searching for time: "+req.query.time+" in Db.");
                result = itemsfile.filter(
                    item => item.time.substring(0, 5).includes(req.query.time)
                );
            }
            break;
        case 2:
            if (req.query.time.length === 2 && req.query.day) {
                console.log("searching for day: "+req.query.day+" at time:"+
                 req.query.time+ "in Db.");
                result1 = itemsfile.filter(
                    item => item.time.substring(0, 2).includes(req.query.time)
                );
        ...
        case 3:
            if (req.query.time.length === 2 && req.query.day && req.query.month) {
                console.log("searching for month: "+req.query.month+", day:"+
                 req.query.day+" at time: "+req.query.time+" in Db.");
                result1 = itemsfile.filter(
                    item => item.time.substring(0, 2).includes(req.query.time)
                );
                ...
                console.log("searching for month: "+req.query.month+", day: "+req.query.day+
                 "at time: "+req.query.time+" in Db.");
                result1 = itemsfile.filter(
                    item => item.time.substring(0, 5).includes(req.query.time)
                );
                result2 = result1.filter(item => item.day.includes(req.query.day));
